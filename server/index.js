...
app.get('/getRole',(request, response)=>{
...
app.post('/AddDescriptor', (request,response)=>{
    ...
    db.query('INSERT INTO competency_descriptor(Description, Area_id, role_id, Track, Status) VALUES (?,?,?,?,?)',[desc, value, role, track, status],
    (error,result)=>{
        if(error){
            console.log(error);
            response.json({err : error, success : false});
